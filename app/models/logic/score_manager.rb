...
class ScoreManager
  ...
  def display_score_paper
    ...
    @score_paper.each do |key, value|
      ...
      puts "|#{ ' ' * left_padding }#{ value }PTS          Player #{ key }#{ ' ' * right_padding }|"
    end
    ...
    if @ties.length > 1
      ...
      tie_padding = (33 - tie_message.length) / 2.0
	    puts "|#{ ' ' * tie_padding.ceil }#{tie_message}#{ ' ' * tie_padding.floor }|"
	  else
	    puts "|#{ ' ' * winner_padding.ceil }Player #{ @winner[ID] } Won!#{ ' ' * winner_padding.floor }|"
    end
    puts "#{ border }\n\n"
  end
