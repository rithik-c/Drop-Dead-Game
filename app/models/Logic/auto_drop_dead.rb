module Logic
	...
	def self.game_output
	  @game_output
	end
  
	def self.game_output=(value)
	  @game_output = value
	end
	  ...
	  def play_game(sides, dice_count, player_count)
		...
		while player_manager.num_players > 0 do
		  ...
		  if current_player.is_dead == true
			Logic.game_output = Logic.game_output + "\nPLAYER #{ current_player.player_id } HAS DROPPED DEAD WITH #{ score_manager.score_paper[current_player.player_id] } PTS!"
		  else
